const API_URL = (
  import.meta.env.VITE_API_URL || "https://sanskritix.onrender.com/api"
).replace(/\/$/, "");
...
async function request<T>(
  ...
  if (!response.ok) {
    throw new Error(
      data.detail || "SanskritiX server request failed"
    );
  }
...
export const api = {
  ...
  askAI: (
    ...
    request<{ answer: string; city: string }>(
      "/ai/ask",
      {
        method: "POST",
        body: JSON.stringify({
